...
ScanUnifierNode::ScanUnifierNode()
{
  ...
  // Publisher
  topicPub_LaserUnified_ = nh_.advertise<sensor_msgs::LaserScan>(config_.output_scan_topic, 1);
...
void ScanUnifierNode::getParams()
{
  ...
  if(pnh_.getParam("output_scan", out_topic))
  {
    config_.output_scan_topic = out_topic;
  }else{
    config_.output_scan_topic = "scan_unified";
  }
}
...
bool ScanUnifierNode::unifyLaserScans(std::vector<sensor_msgs::LaserScan::ConstPtr> current_scans, sensor_msgs::LaserScan &unified_scan)
{
  ...
  if(!current_scans.empty())
  {
    ...
    unified_scan.angle_increment = current_scans.at(0)->angle_increment;
